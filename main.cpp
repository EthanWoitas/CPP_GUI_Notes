...
int main(int, char**)
{
    ...
    while (!done)
    {
        ...
        if (ImGui::Begin("Test", NULL, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse))
        {
            ...
            // Get the windows size and position
            ...
            ImU32 color_top = IM_COL32(0, 128, 255, 255);    // Blue
            ImU32 color_bottom = IM_COL32(0, 255, 128, 255); // Green
            ...
            ImGui::Checkbox("Check me if cool", &test_bool); // Checkbox configuration (Bool declared above this main loop)
            if (ImGui::Button("Click if cool")) // Button that only toggles test_bool ON
            {
                test_bool = true;
            }
        ...
        if (test_bool == true || test_int == 100) // Coniditions to pop up a new window 
        {
            ImGui::SetNextWindowSize(ImVec2(200.0, 200.0));
            if (ImGui::Begin("You're super cool", NULL))
            {
